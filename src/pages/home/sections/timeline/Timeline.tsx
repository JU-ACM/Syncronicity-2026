...
const timelineSteps = [
  ...
  },
  {
    ...
    duration: "16th May",
    description: "Selected teams advance to the prelims",
  ...
  },
  {
    ...
    description: "Top teams will compete for the title",
...
export default function Timeline() {
  ...
  useEffect(() => {
    ...
    mm.add("(min-width: 768px)", () => {
      ...
      checkpointsRef.current.forEach((checkpoint) => {
        if (checkpoint) {
          gsap.set(checkpoint, { opacity: 0.2, color: "white", force3D: true });
        }
      });

      // Track the last scroll direction to prevent tween spam
      let lastDirection = 1;
      ...
      const tl = gsap.timeline({
        scrollTrigger: {
          ...
          scrub: 1,
          onUpdate: (self) => {
            // ONLY fire the tween if the scroll direction actually changes
            if (self.direction !== lastDirection && planeIconRef.current) {
              ...
              gsap.to(planeIconRef.current, {
                ...
                ease: "power2.out",
                transformOrigin: "center center",
                overwrite: true, // Kills the old tween instantly if changed again quickly
              });
            }
          },
        },
      ...
      // Pre-bind functions to avoid recreating them in the loop
      const animateEl = (el: HTMLElement | null, props: gsap.TweenVars) => {
        if (!el) return;
        gsap.to(el, {
          ...props,
          overwrite: "auto",
      ...
      checkpointsRef.current.forEach((checkpoint, index) => {
        ...
        if (checkpoint || circle) {
          // Add a tiny tween to act as a trigger point on the scrubbed timeline
          tl.to(
            {},
            {
              duration: 0.001,
              onStart: () => {
                animateEl(checkpoint, {
                  opacity: 1,
                  color: "#00DB96",
                  duration: 0.4,
                  ease: "power2.out",
  ...
  return (
    <section className="timeline-class relative z-60 w-full overflow-hidden m-0 p-0 text-white font-sans bg-[#131313]">
      <div
        ref={containerRef}
      ...
      >
        ...
        {/* Added will-change-transform here to offload the movement to the GPU */}
        <div
          ref={planeRef}
          className="absolute z-20 w-[47px] h-[43px] pointer-events-none origin-center will-change-transform"
        >
          <svg
            ref={planeIconRef}
            viewBox="0 0 47 43"
        ...
        {timelineSteps.map((step, index) => (
          <div key={step.id}>
            <div
              ...
              className="absolute w-[45px] h-[45px] lg:w-[60px] lg:h-[60px] rounded-full z-10 pointer-events-none will-change-transform"
              style={{
            ...
            >
              ...
              <div className="flex flex-col gap-1.5">
                <h3
                  className={`m-0 text-xl font-unbounded font-semibold ${step.number === "5" ? "lg:text-6xl" : "lg:text-2xl"} `}
                >
                  {step.title}
                </h3>
                <hr className="w-full border-t border-gray-600 my-1" />
                <span
                  className={` font-euclid font-medium tracking-wide ${step.number === "5" ? "text-2xl" : "text-lg"} `}
                >
                  {step.duration}
                </span>
                <span
                  className={`text-sm font-euclid text-gray-400 leading-tight ${step.number === "5" ? "lg:text-xl" : "lg:text-base"} `}
                >
                  {step.description}
                </span>
              </div>
